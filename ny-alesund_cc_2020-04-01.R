...
## TA ##
########### Binding different AT instruments in one column ########### 
data <- data %>%
...
## seaFET ##
########### Binding different seafets in one column ########### 
data <- data %>%
...
#### Argo Global Range Test - Used the range observed rounded to the highest integer
# If QF = 4 replace value by NA
data <- data %>%
  mutate(
    sal_fb_qf = ifelse(data$sal_fb < 30 | data$sal_fb > 36, 4, 1),
    sal_fb = ifelse(sal_fb_qf == 4, NA, data$sal_fb),
    temp_fb_qf = ifelse(data$temp_fb <- -2 | data$temp_fb > 10, 4, 1),
    temp_fb = ifelse(temp_fb_qf == 4, NA, data$temp_fb),
    temp_insitu_11m_qf = ifelse(data$temp_insitu_11m <- -2 | data$temp_insitu_11m > 10, 4, 1),
    temp_insitu_11m = ifelse(temp_insitu_11m_qf == 4, NA, data$temp_insitu_11m),
    temp_insitu_ctd_qf = ifelse(data$temp_insitu_ctd <- -2 | data$temp_insitu_ctd > 10, 4, 1),
    temp_insitu_ctd = ifelse(temp_insitu_ctd_qf == 4, NA, data$temp_insitu_ctd),
    sal_insitu_ctd_qf = ifelse(data$sal_insitu_ctd < 30 |
                                 data$sal_insitu_ctd > 36, 4, 1),
    sal_insitu_ctd = ifelse(sal_insitu_ctd_qf == 4, NA, temp_insitu_ctd),    
    pressure_insitu_ctd_qf = ifelse(data$pressure_insitu_ctd < 13 | 
