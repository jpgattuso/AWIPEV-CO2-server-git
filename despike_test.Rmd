...
#                 pco2_raw=   ifelse(pco2_inst == "0215", pco2_raw_0215,
...
# 
# # Continue to calculate parameters and FINAL PCO2 CORRECTED (pco2_corr)
# data <- data %>%
...
data <- data %>%
  dplyr::mutate( temp_fb = ifelse(temp_dur > 10 | temp_dur < -2 , NA, temp_dur))
...
d <- res %>%
  ...
  pivot_longer(-datetime, names_to = "params", values_to = var) %>%
  dplyr::mutate(params = factor(params)) %>%
  dplyr::mutate(params = factor(params, levels=levels(params)[c(4, 3, 1, 2, 5, 6)]))
plt_n0.5 <- d %>%
  ggplot() +
  geom_point(aes(x=datetime, y=var, col=params), na.rm= TRUE, size=0.25) +
  facet_wrap(vars(params), ncol = 1) +
  theme_bw()
...
dygraph(d_hour_xts,
  ...
  dyAxis("y", valueRange = c(min(var), max(var))) %>%
...
dygraphs_despike(var="data$pco2_corr")
```
...
```
One keeps **n1k8641** which adds less NAs and seems satisfactory.
...
<!-- **n1k8641** adds les NAs than **n1k5761**. To revisit with n2. -->
...
dygraphs_despike(var="data$temp_dur")
```
