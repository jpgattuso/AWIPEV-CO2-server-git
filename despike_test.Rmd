...
This was used in April 2020 to determine the best way to despike the data.
...
* Use of despike:
    ...
    + all combinations of the following n and k values:
        * n <- c(0.5, 1, 2) # number of times sd [0.01, 0.1 have also been tried but too stringent]
...
plt_n0.5 <- d %>%
  ...
  facet_wrap(vars(params), ncol = 1) +
  theme_bw()
#ggsave(filename = paste0(var,"_plt_n0.5"), plt_n0.5, device = "pdf", dpi=150)
print(plt_n0.5)
...
plt_n1 <- d %>%
  ...
  facet_wrap(vars(params), ncol = 1) +
  theme_bw()
#ggsave(filename = paste0(var,"_plt_n1"), plt_n1, device = "pdf", dpi=150)
print(plt_n1)
...
plt_n2 <- d %>%
  ...
  facet_wrap(vars(params), ncol = 1) +
  theme_bw()
#ggsave(filename = paste0(var,"_plt_n2"), plt_n2, device = "pdf", dpi=150)
print(plt_n2)
}
```
...
d_hour <- d %>%
  group_by_key() %>%
  index_by(hour = ~ lubridate::floor_date(., "1 hour")) %>% # hourly aggregates
  summarise(
